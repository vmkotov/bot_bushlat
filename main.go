package main

import (
	"log"
	"net/http"
	"os"
	"strconv"
	"time"

	tgbotapi "github.com/go-telegram-bot-api/telegram-bot-api/v5"
	"github.com/joho/godotenv"
	"github.com/vmkotov/telelog"

	"bushlatinga_bot/bot"
	"bushlatinga_bot/database"
)

func main() {
	//======================================================
	// БЛОК 1: НАЧАЛО РАБОТЫ И ЗАГРУЗКА КОНФИГУРАЦИИ
	//======================================================
	log.Println("🔧 Запускаю Bushlatinga Bot v3.0 (Модульная архитектура)...")

	// Загружаем переменные окружения из файла .env
	if err := godotenv.Load(); err != nil {
		log.Printf("⚠️ Внимание: Файл .env не найден: %v", err)
	}

	//======================================================
	// БЛОК 2: НАСТРОЙКА И ИНИЦИАЛИЗАЦИЯ ТЕЛЕГРАМ БОТА
	//======================================================

	// Получаем токен бота из переменных окружения
	token := os.Getenv("TELEGRAM_BOT_TOKEN")
	if token == "" {
		log.Fatal("❌ TELEGRAM_BOT_TOKEN не найден в .env")
	}

	// Создаем экземпляр бота Telegram API
	botAPI, err := tgbotapi.NewBotAPI(token)
	if err != nil {
		log.Fatalf("❌ Ошибка создания бота: %v", err)
	}

	// Настраиваем режим отладки
	botAPI.Debug = os.Getenv("DEBUG") == "true"
	log.Printf("✅ Авторизован как @%s (ID: %d)", botAPI.Self.UserName, botAPI.Self.ID)

	//======================================================
	// БЛОК 3: ХАРДКОД НАСТРОЕК ДЛЯ ПЕРЕСЫЛКИ СООБЩЕНИЙ
	//======================================================

	// ХАРДКОД: ID чата для пересылки сообщений (Чат B: Архив)
	forwardChatID := int64(-1003669576601)
	log.Printf("🎯 Хардкод ID чата для пересылки (Чат B): %d", forwardChatID)

	//======================================================
	// БЛОК 4: ХАРДКОД ДЛЯ TELELOGGER (Чат A: Важные уведомления)
	//======================================================

	// ХАРДКОД: ID чата для TeleLogger (Чат A: Важные уведомления)
	teleLoggerChatID := int64(-1003459160643) // ← ВАШ НОВЫЙ ЧАТ!
	log.Printf("🎯 Хардкод ID чата для TeleLogger (Чат A): %d", teleLoggerChatID)

	//======================================================
	// БЛОК 5: ИНИЦИАЛИЗАЦИЯ TELELOGGER С ХАРДКОДОМ
	//======================================================

	var teleLogger telelog.TeleLogger

	// TeleLogger идет в Чат A (-1003459160643)
	if teleLoggerChatID != 0 {
		teleLogger = telelog.New(telelog.Options{
			Bot:         botAPI,
			LogChatID:   teleLoggerChatID, // ← ХАРДКОД -1003459160643
			BotID:       botAPI.Self.ID,
			BotUsername: botAPI.Self.UserName,
		})
		log.Printf("✅ TeleLogger инициализирован для Чат А (ID: %d)", teleLoggerChatID)

		// Проверяем доступность чата
		if teleLogger.IsEnabled() {
			testMsg := tgbotapi.NewMessage(teleLoggerChatID,
				"🔄 *Bushlatinga Bot запущен!*\n\n"+
					"📊 *Распределение логов:*\n"+
					"• Этот чат: Важные уведомления (TeleLogger)\n"+
					"• Чат B (-1003669576601): Архив сообщений\n"+
					"• Чат C (-1003585352063): Детальные логи")
			testMsg.ParseMode = "Markdown"

			if _, err := botAPI.Send(testMsg); err != nil {
				log.Printf("⚠️ Не удалось отправить тест в Чат А %d: %v", teleLoggerChatID, err)
			} else {
				log.Printf("✅ Тестовое сообщение отправлено в Чат А")
			}
		}
	} else {
		teleLogger = telelog.SimpleNew()
		log.Println("⚠️ ID чата для TeleLogger равен 0, использую консольный логгер")
	}

	//======================================================
	// БЛОК 6: ИНИЦИАЛИЗАЦИЯ БАЗЫ ДАННЫХ
	//======================================================

	var dbHandler *database.BotDatabaseHandler
	dbURL := os.Getenv("DATABASE_URL")

	if dbURL != "" {
		adminID := int64(266468924)

		if adminEnv := os.Getenv("ADMIN_CHAT_ID"); adminEnv != "" {
			if id, err := strconv.ParseInt(adminEnv, 10, 64); err == nil {
				adminID = id
				log.Printf("👑 Администратор из .env: %d", adminID)
			}
		}

		var err error
		dbHandler, err = database.NewBotDatabaseHandler(adminID, dbURL)
		if err != nil {
			log.Printf("❌ Ошибка инициализации обработчика БД: %v", err)
			log.Printf("⚠️ Бот будет работать без базы данных")
		} else {
			defer dbHandler.Close()
			log.Printf("✅ Обработчик БД инициализирован")
		}
	} else {
		log.Println("ℹ️ DATABASE_URL не установлен, бот работает без базы данных")
	}

	//======================================================
	// БЛОК 7: ИНИЦИАЛИЗАЦИЯ ПЕРЕСЫЛКИ СООБЩЕНИЙ (MESSAGE FORWARDER)
	//======================================================

	var messageForwarder *bot.MessageForwarder

	if forwardChatID != 0 {
		messageForwarder = bot.NewMessageForwarder(botAPI, forwardChatID)
		log.Printf("✅ MessageForwarder инициализирован для Чат B")
		log.Printf("   🎯 Целевой чат ID: %d", forwardChatID)
		log.Printf("   📨 Все сообщения будут пересылаться в этот чат (Архив)")

		// Тестовое сообщение в Чат B
		testMsg := tgbotapi.NewMessage(forwardChatID,
			"🔄 *MessageForwarder запущен!*\n\n"+
				"📋 *Назначение:*\n"+
				"Этот чат для АРХИВА всех входящих сообщений.\n"+
				"Здесь сохраняются оригинальные сообщения от пользователей.")
		testMsg.ParseMode = "Markdown"

		if _, err := botAPI.Send(testMsg); err != nil {
			log.Printf("⚠️ Не удалось отправить тест в Чат B %d: %v", forwardChatID, err)
		} else {
			log.Printf("✅ Тестовое сообщение отправлено в Чат B")
		}
	} else {
		log.Println("⚠️ ID чата для пересылки равен 0, MessageForwarder не создан")
	}

	//======================================================
	// БЛОК 8: СОЗДАНИЕ ОБРАБОТЧИКА ТЕЛЕГРАМ ВЕБХУКОВ
	//======================================================

	telegramHandler := bot.NewTelegramHandler(
		botAPI,
		dbHandler,
		teleLogger,
		messageForwarder,
	)

	log.Println("✅ Обработчик Telegram вебхуков создан")

	//======================================================
	// БЛОК 9: НАСТРОЙКА HTTP СЕРВЕРА И РОУТИНГА
	//======================================================

	http.HandleFunc("/", telegramHandler.HandleWebhook)

	port := os.Getenv("PORT")
	if port == "" {
		port = "8080"
		log.Printf("⚠️ PORT не установлен, использую порт по умолчанию: %s", port)
	}

	//======================================================
	// БЛОК 10: ОТПРАВКА УВЕДОМЛЕНИЯ О ЗАПУСКЕ
	//======================================================

	if teleLogger != nil && teleLogger.IsEnabled() {
		deployInfo := map[string]string{
			"version":         "3.0.0",
			"environment":     getEnvOrDefault("ENVIRONMENT", "production"),
			"branch":          getEnvOrDefault("BRANCH", "main"),
			"commit_hash":     getEnvOrDefault("COMMIT_HASH", "unknown"),
			"deployer":        "Bushlatinga Bot",
			"timestamp":       time.Now().Format("2006-01-02 15:04:05"),
			"forward_chat":    strconv.FormatInt(forwardChatID, 10),
			"telelogger_chat": strconv.FormatInt(teleLoggerChatID, 10),
			"note":            "🚀 Продакшен конфигурация активирована",
		}

		teleLogger.SendDeployNotification(deployInfo)
		log.Println("🚀 Уведомление о деплое отправлено в Чат А")
	} else {
		log.Println("⚠️ TeleLogger не включен, уведомление о деплое не отправлено")
	}

	//======================================================
	// БЛОК 11: ЗАПУСК HTTP СЕРВЕРА
	//======================================================

	log.Printf("🌐 Запускаю HTTP сервер на порту %s", port)
	log.Println("======================================================")
	log.Println("🤖 Бот готов к работе! (Продакшен конфигурация)")
	log.Println("📊 РАСПРЕДЕЛЕНИЕ ЛОГОВ:")
	log.Println("   📍 Чат А (TeleLogger):", teleLoggerChatID, "- Важные уведомления")
	log.Println("   📍 Чат B (Forwarder):", forwardChatID, "- Архив сообщений")
	log.Println("   📍 Чат C (DBLogger): -1003585352063 - Детальные логи")
	log.Println("======================================================")

	if err := http.ListenAndServe(":"+port, nil); err != nil {
		log.Fatalf("❌ Не удалось запустить сервер: %v", err)
	}
}

//======================================================
// ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ
//======================================================

func getEnvOrDefault(key, defaultValue string) string {
	if value := os.Getenv(key); value != "" {
		return value
	}
	return defaultValue
}

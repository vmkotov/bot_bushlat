...
func main() {
	...
	//======================================================
	// БЛОК 5: ХАРДКОД ДЛЯ ДЕПЛОЙ УВЕДОМЛЕНИЙ (Чат D: Деплой)
	//======================================================

	// ХАРДКОД: ID чата для уведомлений о деплое (Чат D: Деплой)
	deployChatID := int64(-1003685049289)
	log.Printf("🎯 Хардкод ID чата для деплой-уведомлений (Чат D): %d", deployChatID)

	//======================================================
	// БЛОК 6: ИНИЦИАЛИЗАЦИЯ TELELOGGER С ХАРДКОДОМ
	//======================================================
		...
		if teleLogger.IsEnabled() {
			testMsg := tgbotapi.NewMessage(teleLoggerChatID,
				"🔄 *Bushlatinga Bot запущен!*\n\n"+
					...
					"• Чат D (-1003685049289): Уведомления о деплое")
	...
	//======================================================
	// БЛОК 7: ИНИЦИАЛИЗАЦИЯ БАЗЫ ДАННЫХ
	//======================================================
	...
	//======================================================
	// БЛОК 8: ИНИЦИАЛИЗАЦИЯ ПЕРЕСЫЛКИ СООБЩЕНИЙ (MESSAGE FORWARDER)
	//======================================================
	...
	//======================================================
	// БЛОК 9: СОЗДАНИЕ ОБРАБОТЧИКА ТЕЛЕГРАМ ВЕБХУКОВ
	//======================================================
	...
	//======================================================
	// БЛОК 10: НАСТРОЙКА HTTP СЕРВЕРА И РОУТИНГА
	//======================================================
	...
	//======================================================
	// БЛОК 11: ОТПРАВКА УВЕДОМЛЕНИЯ О ДЕПЛОЕ В ОТДЕЛЬНЫЙ ЧАТ
	//======================================================
	...
	if deployChatID != 0 && botAPI != nil {
		...
		if _, err := botAPI.Send(deployMsg); err != nil {
			log.Printf("❌ Не удалось отправить уведомление о деплое в чат %d: %v", deployChatID, err)
		} else {
			log.Printf("✅ Уведомление о деплое отправлено в Чат D (%d)", deployChatID)
		}
	}

	//======================================================
	// БЛОК 12: ЗАПУСК HTTP СЕРВЕРА
	//======================================================
	...
	log.Println("   📍 Чат D (Deploy):", deployChatID, "- Уведомления о деплое")
	log.Println("======================================================")
